...
export function SettingsView({ settings, transcription, onChange, onClose, t }: Props) {
  ...
  return (
    <div className="settings">
      ...
      <label className="field">
        <span className="label">{t('settings.spokenLanguage')}</span>
        <select value={settings.language} onChange={(event) => onChange({ language: event.target.value })}>
          <option value="auto">
            {transcription.detected
              ? `${t('lang.auto')} · ${LANGUAGES.find((l) => l.code === transcription.detected)?.flag ?? ''} ${transcription.detected}`
              : t('lang.auto')}
          </option>
